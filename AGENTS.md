...
Deepsuck is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Deepsuck ships a **lot** — most merges are bug fixes to
...
Applies to TypeScript across Deepsuck: desktop, TUI, website, and future TS packages.
...
```
2. Add handler in `DeepsuckCLI.process_command()` in `cli.py`:
```python
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Deepsuck built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
core Deepsuck tool that should ship in the base system.
...
- `default` — Classic Deepsuck gold/kawaii (the current look)
...
2. **Tools referenced in SKILL.md prose must be native Deepsuck tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Deepsuck Agent" is the secondary collaborator. If the
   contributor's commit shows "Deepsuck Agent" as author (because they
   used Deepsuck to draft the skill), replace it with their actual name
...
Deepsuck-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
