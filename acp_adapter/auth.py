"""ACP auth helpers — detect and advertise Dag authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "dag-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Dag runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Dag can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Dag.
    ...
    not have Dag provider credentials configured yet, so Dag always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Dag using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Dag provider",
            description=(
                "Open Dag' interactive model/provider setup in a terminal. "
                "Use this when Dag has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
