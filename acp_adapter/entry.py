"""CLI entry point for the dag-agent ACP adapter.

Loads environment variables from ``~/.dag/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from DAG_HOME (default ``~/.dag``)."""
    from dag_cli.env_loader import load_dag_dotenv

    dag_home = get_dag_home()
    loaded = load_dag_dotenv(dag_home=dag_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", dag_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="dag-acp",
        description="Run Dag Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Dag version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import DagACPAgent  # noqa: F401

    print("Dag ACP check OK")


def _run_setup() -> None:
    from dag_cli.main import main as dag_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "dag", "model"]
        dag_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``dag postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting dag-agent ACP adapter")
    ...
    import acp
    from .server import DagACPAgent
