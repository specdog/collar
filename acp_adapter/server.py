"""ACP agent server — exposes Dag Agent via the Agent Client Protocol."""
...
try:
    from dag_cli import __version__ as DAG_VERSION
except Exception:
    DAG_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Dag running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Dag cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Dag/OpenAI-compatible user content payload."""
...
class DagACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Dag AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Dag maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from dag_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from dag_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Dag estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_dag_session_id: str,
        previous_dag_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.dag.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_dag_session_id,
                previous_dag_session_id=previous_dag_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_dag_session_id: Optional[str] = None,
        previous_dag_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Dag changes it.

        When the internal Dag head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_dag_session_id`` so the
        attached ``_meta.dag.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # dag_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_dag_session_id or session_id,
            previous_dag_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["dag-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="dag-agent", version=DAG_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Dag' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Dag setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Dag
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Dag on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Dag' local kawaii waiting/status
        ...
        # not the executor's. Also set DAG_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue DAG_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind DAG_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("DAG_INTERACTIVE")
            os.environ["DAG_INTERACTIVE"] = "1"
            ...
            previous_session_id = os.environ.get("DAG_SESSION_ID")
            os.environ["DAG_SESSION_ID"] = session_id
            try:
                ...
                # Restore DAG_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("DAG_INTERACTIVE", None)
                else:
                    os.environ["DAG_INTERACTIVE"] = previous_interactive
                # Restore DAG_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("DAG_SESSION_ID", None)
                else:
                    os.environ["DAG_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Dag DB session id before the turn so we
            ...
            pre_turn_dag_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (DAG_SESSION_KEY in
        ...
        # _meta.dag.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_dag_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_dag_id
            and pre_turn_dag_id
            and post_turn_dag_id != pre_turn_dag_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_dag_session_id=post_turn_dag_id,
                    previous_dag_session_id=pre_turn_dag_id,
                )
            except Exception:
                logger.debug(
        ...
        # Dag' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Dag Agent v{DAG_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Dag has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
