"""Anthropic Messages API adapter for DAG Agent.

Translates between Dag's internal OpenAI-style message format and
...
# Dag effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Dag historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Dag)
    ...
    # 1. Dag-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Dag configs.
...
# ── Dag-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.dag/.anthropic_oauth.json (our own file).
...
def run_dag_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Dag-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Dag with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from dag_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_dag_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _DEEPSUCK_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Dag OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See dag-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See dag-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See dag-agent#13848 (Kimi) and
    dag-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("DAG Agent", "Claude Code")
                text = text.replace("DAG agent", "Claude Code")
                text = text.replace("dag-agent", "claude-code")
    ...
    # silently hides reasoning text that Dag surfaces in its CLI. We
