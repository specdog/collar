"""Anthropic Messages API adapter for Deepsuck Agent.

Translates between Deepsuck's internal OpenAI-style message format and
...
# Deepsuck effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See deepsuck-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Deepsuck historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Deepsuck)
    ...
    # 1. Deepsuck-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Deepsuck configs.
...
# ── Deepsuck-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.deepsuck/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_DEEPSUCK_OAUTH_FILE = get_deepsuck_home() / ".anthropic_oauth.json"
...
def run_deepsuck_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Deepsuck-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Deepsuck with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from deepsuck_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_deepsuck_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Deepsuck-managed OAuth credentials from ~/.deepsuck/.anthropic_oauth.json."""
    if _DEEPSUCK_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Deepsuck OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See deepsuck-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See deepsuck-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See deepsuck-agent#13848 (Kimi) and
    deepsuck-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Deepsuck Agent", "Claude Code")
                text = text.replace("Deepsuck agent", "Claude Code")
                text = text.replace("deepsuck-agent", "claude-code")
    ...
    # silently hides reasoning text that Deepsuck surfaces in its CLI. We
