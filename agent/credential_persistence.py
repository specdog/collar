...
``auth.json``.  They intentionally have no dependency on ``dag_cli.auth`` so
...
# Sources Dag owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and DAG-owned OAuth/device-code state)
