...
``auth.json``.  They intentionally have no dependency on ``deepsuck_cli.auth`` so
...
# Sources Deepsuck owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Deepsuck-owned OAuth/device-code state)
