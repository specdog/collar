...
as ``kind: backend``) or ``~/.dag/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$DAG_HOME/cache/images/``, creating parents as needed."""
    from dag_constants import get_dag_home

    path = get_dag_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$DAG_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$DAG_HOME/cache/images/``.
