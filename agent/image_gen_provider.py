...
as ``kind: backend``) or ``~/.deepsuck/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$DEEPSUCK_HOME/cache/images/``, creating parents as needed."""
    from deepsuck_constants import get_deepsuck_home

    path = get_deepsuck_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$DEEPSUCK_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$DEEPSUCK_HOME/cache/images/``.
