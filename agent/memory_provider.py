...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - dag_home (str): The active DAG_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.dag``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "dag").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'dag memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], dag_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'dag memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``dag_home`` is the active DAG_HOME directory path.
