...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Dag also honours DEEPSUCK_CA_BUNDLE (its own convention)
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_dag_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run DAG Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Dag from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Dag' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from dag_constants import get_dag_home
    return get_dag_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Dag grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from dag_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Dag sends to providers:
