...
Plugins built on Deepsuck Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Deepsuck
...
:class:`~deepsuck_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Deepsuck' main config
...
handles every provider, fallback chain, and per-task override Deepsuck
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`deepsuck_cli.plugins.PluginContext`
