...
Plugins built on DAG Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Dag
...
:class:`~dag_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Dag' main config
...
handles every provider, fallback chain, and per-task override Dag
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`dag_cli.plugins.PluginContext`
