...
Every Dag request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to DAG Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=dag-agent",
        "client=dag-client-v<__version__>",
    ]

The version is sourced live from ``dag_cli.__version__`` so it auto-aligns
...
``dag_cli.__version__`` is the canonical source of truth.
"""
...
def _dag_version() -> str:
    """Return the current Dag release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``dag_cli`` cannot be imported (should
...
def dag_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=dag-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=dag-client-v{_dag_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=dag-agent", dag_client_tag()]
