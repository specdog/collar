...
Every Deepsuck request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Deepsuck Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=deepsuck-agent",
        "client=deepsuck-client-v<__version__>",
    ]

The version is sourced live from ``deepsuck_cli.__version__`` so it auto-aligns
...
``deepsuck_cli.__version__`` is the canonical source of truth.
"""
...
def _deepsuck_version() -> str:
    """Return the current Deepsuck release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``deepsuck_cli`` cannot be imported (should
...
def deepsuck_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=deepsuck-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=deepsuck-client-v{_deepsuck_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=deepsuck-agent", deepsuck_client_tag()]
