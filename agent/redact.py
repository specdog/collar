...
# (bridged to this env var in dag_cli/main.py, gateway/run.py, and
# cli.py) or `DEEPSUCK_REDACT_SECRETS=false` in ~/.dag/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Dag — used by
    ``dag config``, ``dag status``, ``dag dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical dag log line
