...
Dag pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.dag/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<dag_home>/bin/bws`` on
  first use.  Dag pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.dag/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``dag`` invocations don't
  hammer the API.
* Failures NEVER block Dag startup.  Missing binary, no network,
...
# In-process cache so repeated load_dag_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `dag chat -q ...`
...
# <dag_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.dag/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under dag_home/cache/.

    `home_path` is what `load_dag_dotenv()` already resolved; falling back
    to `$DAG_HOME` / `~/.dag` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("DAG_HOME", Path.home() / ".dag"))
...
def _dag_bin_dir() -> Path:
    """Where Dag stores its managed binaries.  Profile-aware."""
    from dag_constants import get_dag_home

    return get_dag_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<dag_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _dag_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``dag secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _dag_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="dag-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "dag-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<dag_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from dag_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_dag_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `dag secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `dag secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `dag secrets bitwarden setup` to install."
        )
        return result
