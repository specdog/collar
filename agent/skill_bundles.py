...
Storage
-------
Bundles live in ``~/.deepsuck/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``DEEPSUCK_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``deepsuck bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under DEEPSUCK_HOME.

    Honors ``DEEPSUCK_BUNDLES_DIR`` for tests; falls back to
    ``<DEEPSUCK_HOME>/skill-bundles``.
    """
    override = os.environ.get("DEEPSUCK_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_deepsuck_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `deepsuck bundles` CLI subcommand.
# ---------------------------------------------------------------------------
