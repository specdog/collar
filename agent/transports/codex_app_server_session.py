...
Owns one Codex thread per Deepsuck session. Drives `turn/start`, consumes
...
# Deepsuck' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Deepsuck/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Deepsuck session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="deepsuck",
            client_title="Deepsuck Agent",
            client_version=_get_deepsuck_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Deepsuck' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Deepsuck' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own deepsuck-tools callback we
            # auto-accept — the user already approved Deepsuck' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Deepsuck approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Deepsuck returns 'once', 'session', 'always', or 'deny'.
...
def _get_deepsuck_version() -> str:
    """Best-effort Deepsuck version string for codex's userAgent line."""
    try:
