...
Owns one Codex thread per Dag session. Drives `turn/start`, consumes
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Dag/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Dag session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="dag",
            client_title="DAG Agent",
            client_version=_get_dag_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Dag' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Dag' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own dag-tools callback we
            # auto-accept — the user already approved Dag' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Dag approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Dag returns 'once', 'session', 'always', or 'deny'.
...
def _get_dag_version() -> str:
    """Best-effort Dag version string for codex's userAgent line."""
    try:
