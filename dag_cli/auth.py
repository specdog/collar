...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("DAG_OAUTH_TRACE", "").strip().lower()
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("DAG_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("DAG_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set DAG_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("DAG_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("DAG_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("DAG_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # into the log file.  Run with ``DAG_LOG_LEVEL=INFO`` (or check
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    save_env_value("DAG_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("DAG_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved DAG_SPOTIFY_CLIENT_ID to ~/.dag/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "DAG_SPOTIFY_CLIENT_ID is required".
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("DAG_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DAG_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``DAG_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DAG_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
...
#
# File lives at ${DAG_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``DAG_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    ...
    """
    override = os.getenv("DAG_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set DAG_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set DAG_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("DAG_PORTAL_BASE_URL")
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    raw_args = os.getenv("DAG_COPILOT_ACP_ARGS", "").strip()
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("DAG_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
