...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``DAG_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("DAG_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # DAG_REVISION (that's nix-only). Without this guard the checks below
