...
# * ``DAG_HOME`` / ``DAG_PROFILE`` / ``DAG_CONFIG`` /
#   ``DAG_ENV`` — Dag runtime location flags. Writing these into
...
#
# IMPORTANT: ``DAG_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (DAG_GEMINI_CLIENT_ID,
# DAG_LANGFUSE_PUBLIC_KEY, DAG_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a DAG_* blanket: integration credentials (DAG_GEMINI_*,
    # DAG_LANGFUSE_*, DAG_SPOTIFY_*, ...) ARE allowed.
    "DAG_HOME", "DAG_PROFILE", "DAG_CONFIG", "DAG_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(DAG_HOME, DAG_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "DAG_TOOL_PROGRESS", "DAG_TOOL_PROGRESS_MODE",
    ...
    "DAG_LANGFUSE_ENV",
    "DAG_LANGFUSE_RELEASE",
    "DAG_LANGFUSE_SAMPLE_RATE",
    "DAG_LANGFUSE_MAX_CHARS",
    "DAG_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("DAG_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the DAG_MANAGED env var (set by the systemd service),
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. DAG_MANAGED env / .managed marker (NixOS, Homebrew)
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container, or DAG_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("DAG_DEV") == "1":
        return None
...
def _resolve_dag_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the DAG_UID / DAG_GID env vars set by Docker deployments.
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("DAG_UID", "").strip()
    gid_str = os.environ.get("DAG_GID", "").strip()
    try:
...
def _chown_to_dag_uid(path) -> None:
    """Chown ``path`` to ``DAG_UID:DAG_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    Also applies ``DAG_UID``/``DAG_GID``-based ownership when those env
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("DAG_CONTAINER") or os.environ.get("DAG_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set DAG_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The DAG_ENVIRONMENT_HINT
    ...
    "dashboard": {
        ...
        # ``DAG_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``DAG_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``DAG_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``DAG_DASHBOARD_PUBLIC_URL``).
    ...
    "skills": {
        ...
        # Substitute ${DAG_SKILL_DIR} and ${DAG_SESSION_ID} in SKILL.md
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (DAG_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # toggleable per-invocation via --accept-hooks or DAG_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via DAG_CRON_MAX_PARALLEL env var.
    ...
    "gateway": {
        ...
        # user. Bridged to DAG_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # to DAG_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # DAG_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to DAG_MEDIA_TRUST_RECENT_SECONDS.
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. DAG_BOT)",
    ...
    # DAG_TOOL_PROGRESS and DAG_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("DAG_TOOL_PROGRESS")
            old_mode = get_env_value("DAG_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from DAG_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from DAG_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("DAG_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from DAG_TIMEZONE)")
            else:
                config["timezone"] = ""
...
def show_config():
    ...
    # Warn on stale DAG_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("DAG_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale DAG_MAX_ITERATIONS={_env_ghost} "
