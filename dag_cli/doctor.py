...
from dag_cli.models import _DAG_USER_AGENT
...
def run_doctor(args):
    ...
    os.environ.setdefault("DAG_INTERACTIVE", "1")
    ...
    if config_path.exists():
        ...
        # Detect stale DAG_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives DAG_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            env_ghost = load_env().get("DAG_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"DAG_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `dag setup` run)",
                )
                if should_fix:
                    if remove_env_value("DAG_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale DAG_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove DAG_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the DAG_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale DAG_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'dag doctor --fix'"
                    )
        except Exception:
            pass
