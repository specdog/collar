...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("DAG_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("DAG_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("DAG_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["DAG_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["DAG_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["DAG_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("DAG_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("DAG_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("DAG_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def _build_gateway_cmd_script(
    ...
    lines.append('set "DAG_GATEWAY_DETACHED=1"')
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("DAG_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("DAG_GATEWAY_INSTALL_START_ON_LOGIN")
