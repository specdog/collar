...
def _resolve_dag_argv() -> list[str]:
    ...
    1. ``$DAG_BIN`` — explicit operator override. Path-like values are
    ...
    env_bin = os.environ.get("DAG_BIN", "").strip()
    if env_bin:
...
def _default_spawn(
    ...
    *,
    ...
    try:
        ...
        # _apply_profile_override() via DAG_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # DAG_HOME never had profiles created.
        pass
    if task.tenant:
        env["DAG_TENANT"] = task.tenant
    ...
    # DAG_PROFILE is the author the kanban_comment tool defaults to.
    # `dag -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["DAG_PROFILE"] = profile_arg
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled DAG_PROFILE values like "dag-system"
