...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``DAG_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("DAG_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``DAG_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("DAG_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _try_termux_ultrafast_version() -> bool:
    ...
    if os.environ.get("DAG_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _apply_profile_override() -> None:
    ...
    # run-script as DAG_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    if profile_name is None and not os.environ.get("DAG_S6_SUPERVISED_CHILD"):
        try:
...
# Bridge security.redact_secrets from config.yaml → DAG_REDACT_SECRETS env
        ...
        if "DAG_REDACT_SECRETS" not in os.environ:
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["DAG_REDACT_SECRETS"] = str(_early_redact).lower()
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("DAG_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("DAG_TERMUX_PREFETCH_UPDATES") == "1"
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``DAG_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("DAG_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``DAG_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("DAG_SKIP_NODE_BOOTSTRAP"):
        return
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (DAG_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("DAG_NODE")
    ...
    ext_dir = os.environ.get("DAG_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with DAG_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset DAG_TUI_DIR (e.g. `unset DAG_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("DAG_QUIET"):
...
def _launch_tui(
    ...
    env["DAG_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["DAG_PYTHON_SRC_ROOT"] = os.environ.get(
        "DAG_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("DAG_PYTHON", sys.executable)
    env.setdefault("DAG_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["DAG_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["DAG_MODEL"] = model
        env["DAG_INFERENCE_MODEL"] = model
    if provider:
        env["DAG_TUI_PROVIDER"] = provider
        env["DAG_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["DAG_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["DAG_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["DAG_TUI_SKILLS"] = value
    if query:
        env["DAG_TUI_QUERY"] = query
    if image:
        env["DAG_TUI_IMAGE"] = image
    if checkpoints:
        env["DAG_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["DAG_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["DAG_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["DAG_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["DAG_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["DAG_ACCEPT_HOOKS"] = "1"
    ...
    # DAG_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set DAG_TUI_RESUME themselves.
    env.pop("DAG_TUI_RESUME", None)
    if resume_session_id:
        env["DAG_TUI_RESUME"] = resume_session_id
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``DAG_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("DAG_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    # plugin/MCP discovery suppression (DAG_SAFE_MODE is checked by
    # dag_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["DAG_SAFE_MODE"] = "1"
        os.environ["DAG_IGNORE_USER_CONFIG"] = "1"
        os.environ["DAG_IGNORE_RULES"] = "1"
...
def select_provider_and_model(args=None):
    ...
    effective_provider = (
        config_provider or os.getenv("DAG_INFERENCE_PROVIDER") or "auto"
    )
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["DAG_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["DAG_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "dag_root", None):
        env["DAG_DESKTOP_DAG_ROOT"] = str(Path(args.dag_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["DAG_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    environment variable ``DAG_DESKTOP_CHILD_PID`` on the spawned
...
def _kill_stale_dashboard_processes(
    ...
    # backend child, it sets DAG_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("DAG_DESKTOP_CHILD_PID")
    if raw_pid:
...
def cmd_dashboard(args):
    ...
    if (
        ...
        and os.environ.get("DAG_DESKTOP") != "1"
    ):
    ...
    if "DAG_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["DAG_WEB_DIST"])
            if "DAG_WEB_DIST" in os.environ
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("DAG_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["DAG_DEFER_AGENT_STARTUP"] = "1"
            os.environ["DAG_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["DAG_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
