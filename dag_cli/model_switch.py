...
_NOUS_DAG_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])dag[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_dag_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_DAG_NON_AGENTIC_RE.search(model_name))
...
def list_authenticated_providers(
    ...
    *,
    ...
    for dag_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (DAG_OVERLAYS) with proper auth store checking.
    ...
    # in PROVIDER_TO_MODELS_DEV or DAG_OVERLAYS (keeps /model in sync
    # with `dag model`).
    try:
