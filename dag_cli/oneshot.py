...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to DAG_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("DAG_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "dag -z: --provider requires --model (or DAG_INFERENCE_MODEL). "
...
def _run_agent(
    ...
    env_model = os.getenv("DAG_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("DAG_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                DAG_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via DAG_ACCEPT_HOOKS=1
