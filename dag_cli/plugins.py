...
# ---------------------------------------------------------------------------
#
# Set ``DAG_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
...
_PLUGINS_DEBUG = os.getenv("DAG_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When DAG_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "DAG_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("DAG_SAFE_MODE"):
            logger.info("DAG_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if _env_enabled("DAG_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set DAG_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
