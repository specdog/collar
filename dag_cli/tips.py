...
TIPS = [
    ...
    "DAG_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "DAG_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "DAG_DEV=1 bypasses container mode detection for local development.",
    ...
    "DAG_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    ...
    # --- TUI & Dashboard ---
    'DAG_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "DAG_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'DAG_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'DAG_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'DAG_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'DAG_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'DAG_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'DAG_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.dag/plugins/ — trust-gated by design.',
    "DAG_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'DAG_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'DAG_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'DAG_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'DAG_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'DAG_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'DAG_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'DAG_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'DAG_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'DAG_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'DAG_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
