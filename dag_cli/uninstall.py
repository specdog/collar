...
#
#   1. Sets User-scope env vars ``DAG_HOME`` and ``DAG_GIT_BASH_PATH``
...
def remove_dag_env_vars_windows() -> list[str]:
    """Delete DAG_HOME and DAG_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("DAG_HOME", "DAG_GIT_BASH_PATH"):
                try:
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        log_info("Removing DAG_HOME / DAG_GIT_BASH_PATH User env vars...")
