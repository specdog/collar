...
WEB_DIST = Path(os.environ["DAG_WEB_DIST"]) if "DAG_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (DAG_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `dag
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("DAG_DESKTOP") == "1":
...
# DAG_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("DAG_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
_MANAGED_FILES_ROOT_ENV = "DAG_DASHBOARD_FILES_ROOT"
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # (the container/hosted layout) or when DAG_DASHBOARD_FILES_ROOT is set.
...
def _spawn_dag_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "DAG_NONINTERACTIVE": "1"},
    }
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        _DAG_OAUTH_FILE = None  # type: ignore
    ...
    if dag_creds and dag_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "dag_pkce",
            "source_label": f"DAG PKCE ({_DAG_OAUTH_FILE})",
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id == "anthropic":
        cleared = False
        try:
            from agent.anthropic_adapter import _DAG_OAUTH_FILE
            if _DAG_OAUTH_FILE.exists():
                _DAG_OAUTH_FILE.unlink()
                cleared = True
        except Exception:
            pass
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _DAG_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _DAG_OAUTH_FILE.with_name(
        f"{_DAG_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _DAG_OAUTH_FILE)
        try:
            _DAG_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``DAG_TUI_RESUME`` env var —
    matching what ``dag_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``DAG_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``DAG_TUI_SIDECAR_URL`` so
    ...
    ``DAG_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    env.setdefault("DAG_TUI_DISABLE_MOUSE", "1")
    env.setdefault("DAG_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["DAG_TUI_RESUME"] = resume

    if sidecar_url:
        env["DAG_TUI_SIDECAR_URL"] = sidecar_url
    ...
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["DAG_TUI_GATEWAY_URL"] = gateway_ws_url
...
# DAG_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__DAG_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__DAG_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__DAG_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__DAG_BASE_PATH__="{prefix}";'
                f"window.__DAG_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.dag/plugins/  (only if DAG_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if env_var_enabled("DAG_ENABLE_PROJECT_PLUGINS"):
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing DAG_DASHBOARD_OAUTH_CLIENT_ID).
    ...
    # DAG_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"DAG_DASHBOARD_READY port={actual_port}", flush=True)
