...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("DEEPSUCK_OAUTH_TRACE", "").strip().lower()
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("DEEPSUCK_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("DEEPSUCK_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set DEEPSUCK_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("DEEPSUCK_SPOTIFY_REDIRECT_URI"),
...
def login_spotify_command(args) -> None:
    ...
    # with "DEEPSUCK_SPOTIFY_CLIENT_ID is required".
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("DEEPSUCK_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DEEPSUCK_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``DEEPSUCK_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DEEPSUCK_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("DEEPSUCK_CA_BUNDLE")
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set DEEPSUCK_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("DEEPSUCK_PORTAL_BASE_URL")
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    raw_args = os.getenv("DEEPSUCK_COPILOT_ACP_ARGS", "").strip()
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("DEEPSUCK_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
