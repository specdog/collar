...
#
# IMPORTANT: ``DEEPSUCK_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (DEEPSUCK_GEMINI_CLIENT_ID,
# DEEPSUCK_LANGFUSE_PUBLIC_KEY, DEEPSUCK_SPOTIFY_CLIENT_ID, ...). The
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "DEEPSUCK_TOOL_PROGRESS", "DEEPSUCK_TOOL_PROGRESS_MODE",
    ...
    "DEEPSUCK_LANGFUSE_SAMPLE_RATE",
    "DEEPSUCK_LANGFUSE_MAX_CHARS",
    "DEEPSUCK_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("DEEPSUCK_MANAGED", "").strip()
    if raw:
...
def _resolve_deepsuck_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("DEEPSUCK_UID", "").strip()
    gid_str = os.environ.get("DEEPSUCK_GID", "").strip()
    try:
...
def _secure_dir(path):
    ...
    Also applies ``DEEPSUCK_UID``/``DEEPSUCK_GID``-based ownership when those env
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("DEEPSUCK_CONTAINER") or os.environ.get("DEEPSUCK_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set DEEPSUCK_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
DEFAULT_CONFIG = {
    ...
    "dashboard": {
        ...
        # ``DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``DEEPSUCK_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``DEEPSUCK_DASHBOARD_BASIC_AUTH_USERNAME`` /
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (DEEPSUCK_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # toggleable per-invocation via --accept-hooks or DEEPSUCK_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via DEEPSUCK_CRON_MAX_PARALLEL env var.
    ...
    "gateway": {
        ...
        # to DEEPSUCK_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # DEEPSUCK_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to DEEPSUCK_MEDIA_TRUST_RECENT_SECONDS.
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. DEEPSUCK_BOT)",
    ...
    # DEEPSUCK_TOOL_PROGRESS and DEEPSUCK_TOOL_PROGRESS_MODE are deprecated —
    ...
    "DEEPSUCK_PREFILL_MESSAGES_FILE": {
    ...
    "DEEPSUCK_EPHEMERAL_SYSTEM_PROMPT": {
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("DEEPSUCK_TOOL_PROGRESS")
            old_mode = get_env_value("DEEPSUCK_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from DEEPSUCK_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from DEEPSUCK_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("DEEPSUCK_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from DEEPSUCK_TIMEZONE)")
            else:
                config["timezone"] = ""
...
def show_config():
    ...
    # Warn on stale DEEPSUCK_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("DEEPSUCK_MAX_ITERATIONS")
