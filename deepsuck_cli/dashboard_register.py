...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``DEEPSUCK_DASHBOARD_PORTAL_URL`` env (used for testing against a
...
def _print_post_register_hint(
    *,
    ...
    print("    DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    DEEPSUCK_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    DEEPSUCK_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
...
def cmd_dashboard_register(args) -> None:
    ...
    # DEEPSUCK_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  DEEPSUCK_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      DEEPSUCK_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("DEEPSUCK_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("DEEPSUCK_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the supplied redirect URI and persist it as DEEPSUCK_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("DEEPSUCK_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("DEEPSUCK_DASHBOARD_PUBLIC_URL", public_url)
