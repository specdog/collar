...
def run_doctor(args):
    ...
    if config_path.exists():
        ...
        # Detect stale DEEPSUCK_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives DEEPSUCK_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            env_ghost = load_env().get("DEEPSUCK_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                ...
                if should_fix:
                    if remove_env_value("DEEPSUCK_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale DEEPSUCK_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove DEEPSUCK_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the DEEPSUCK_MAX_ITERATIONS line from "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _DEEPSUCK_USER_AGENT,
            }
