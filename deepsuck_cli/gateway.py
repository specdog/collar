...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    detached = os.getenv("DEEPSUCK_GATEWAY_DETACHED", "").strip().lower()
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("DEEPSUCK_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``DEEPSUCK_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("DEEPSUCK_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("DEEPSUCK_ALLOW_ROOT_GATEWAY")):
        return
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # DEEPSUCK_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``DEEPSUCK_S6_SUPERVISED_CHILD`` is exported by
         ...
         → ...).
      3. ``--no-supervise`` (or ``DEEPSUCK_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("DEEPSUCK_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("DEEPSUCK_S6_SUPERVISED_CHILD"):
    ...
    print(
        ...
        "  dashboard supervised alongside if DEEPSUCK_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or DEEPSUCK_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if os.getenv("_DEEPSUCK_GATEWAY") == "1":
            print_error(
