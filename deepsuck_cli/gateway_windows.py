...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("DEEPSUCK_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("DEEPSUCK_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("DEEPSUCK_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["DEEPSUCK_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["DEEPSUCK_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["DEEPSUCK_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("DEEPSUCK_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("DEEPSUCK_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("DEEPSUCK_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("DEEPSUCK_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("DEEPSUCK_GATEWAY_INSTALL_START_ON_LOGIN")
