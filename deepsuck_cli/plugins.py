...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``DEEPSUCK_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("DEEPSUCK_BUNDLED_PLUGINS")
    if env_override:
...
_PLUGINS_DEBUG = os.getenv("DEEPSUCK_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    logger.debug(
        "DEEPSUCK_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("DEEPSUCK_SAFE_MODE"):
            logger.info("DEEPSUCK_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if _env_enabled("DEEPSUCK_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set DEEPSUCK_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
