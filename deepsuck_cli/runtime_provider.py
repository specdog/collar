...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("DEEPSUCK_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("DEEPSUCK_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
    ...
    *,
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, env_int("DEEPSUCK_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
