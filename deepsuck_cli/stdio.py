...
on Japanese locales, etc.).  Deepsuck's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``deepsuck_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Deepsuck is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``DEEPSUCK_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("DEEPSUCK_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Deepsuck-managed Git install directories so
    ...
    # already-running shells don't see the change — which means deepsuck
    ...
    # install without Deepsuck-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Deepsuck's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Deepsuck's
    ``deepsuck config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Deepsuck (User env var in Windows Settings, or
    export in a PowerShell profile) and Deepsuck picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Deepsuck-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\deepsuck\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``deepsuck`` from right after install) retain their old PATH.

    Any subprocess Deepsuck spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Deepsuck-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "deepsuck", "git", "cmd"),
        os.path.join(local_appdata, "deepsuck", "git", "bin"),
        os.path.join(local_appdata, "deepsuck", "git", "usr", "bin"),
        # Deepsuck venv Scripts directory — host of the deepsuck.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes deepsuck, but harmless to include.
        os.path.join(local_appdata, "deepsuck", "deepsuck-agent", "venv", "Scripts"),
