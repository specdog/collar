...
on Japanese locales, etc.).  Dag's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``dag_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Dag is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Dag-managed Git install directories so
    ...
    # already-running shells don't see the change — which means dag
    ...
    # install without Dag-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Dag's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Dag's
    ``dag config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Dag (User env var in Windows Settings, or
    export in a PowerShell profile) and Dag picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Dag-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\dag\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``dag`` from right after install) retain their old PATH.

    Any subprocess Dag spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Dag-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "dag", "git", "cmd"),
        os.path.join(local_appdata, "dag", "git", "bin"),
        os.path.join(local_appdata, "dag", "git", "usr", "bin"),
        # Dag venv Scripts directory — host of the dag.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes dag, but harmless to include.
        os.path.join(local_appdata, "dag", "dag-agent", "venv", "Scripts"),
