...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or dag -c.",
    ...
    "Ctrl+Z suspends Dag to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "dag -c resumes your most recent CLI session. dag -c \"project name\" resumes by title.",
    "dag -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "dag -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "dag chat -t web,terminal enables only specific toolsets for a focused session.",
    "dag chat -s github-pr-workflow preloads a skill at launch.",
    "dag chat -q \"query\" runs a single non-interactive query and exits.",
    "dag chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "dag chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "dag --yolo bypasses all dangerous command approval prompts for the entire session.",
    "dag chat --source telegram tags the session for filtering in dag sessions list.",
    "dag -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "dag doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "dag dump outputs a compact setup summary — great for bug reports.",
    "dag config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "dag config edit opens config.yaml in your default editor.",
    "dag config check scans for missing or stale configuration options.",
    "dag sessions browse opens an interactive session picker with search.",
    "dag sessions stats shows session counts by platform and database size.",
    "dag sessions prune --older-than 30 cleans up old sessions.",
    "dag skills search react --source skills-sh searches the skills.sh public directory.",
    "dag skills check scans installed hub skills for upstream updates.",
    "dag skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "dag skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "dag mcp add github --command npx adds MCP servers from the command line.",
    "dag mcp serve runs Dag itself as an MCP server for other agents.",
    "dag auth add lets you add multiple API keys for credential pool rotation.",
    "dag completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "dag logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "dag backup creates a zip backup of your entire Dag home directory.",
    "dag profile create coder creates an isolated profile that becomes its own command.",
    "dag profile create work --clone copies your current config and keys to a new profile.",
    "dag update syncs new bundled skills to ALL profiles automatically.",
    "dag gateway install sets up Dag as a system service (systemd/launchd).",
    "dag memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "dag webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: dag tools disables unused tools, dag skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "dag models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as dag -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call DAG tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'dag profile create coder' creates the 'coder' command.",
    "dag profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Dag prints a resume command with session ID and stats.",
    "dag sessions export backup.jsonl exports all sessions for backup or analysis.",
    "dag -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "dag skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .dag/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: dag cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "DAG runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "dag gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Dag via a local macOS server.",
    ...
    "Context files (.dag.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "DAG loads project context from .dag.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "dag mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "dag mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "dag mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — dag tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.dag/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "dag chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "dag plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "dag acp runs Dag as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "dag auth add nous or dag auth add openai-codex sets up OAuth-based providers.",
    ...
    "dag status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.dag/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Dag your own.",
    ...
    "Before a gateway session reset, Dag auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.dag/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Dag state via DAG_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "DAG_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in DAG_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.dag/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'dag profile create ls' would be rejected.",
    "dag profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "dag claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via dag tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.dag/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.dag/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Dag auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Dag config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Dag is working.',
    ...
    '/reload reloads ~/.dag/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.dag/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.dag/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'dag curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "dag curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'dag curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'dag auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'dag dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'dag dashboard embeds the full Dag TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.dag/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.dag/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.dag/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.dag/skills/ so drop-in skills appear without restarting the session.',
    ...
    'dag -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'dag chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'dag chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'dag chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "dag chat --source tool tags programmatic chats so they don't clutter dag sessions list.",
    'dag dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'dag sessions rename <ID> "new title" renames any past session; dag sessions delete <ID> removes one.',
    'dag import restores a session export or profile archive produced by sessions export or profile export.',
    'dag fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'dag pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'dag setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'dag status --deep runs the full health sweep across every component; plain dag status is the quick view.',
    ...
    'DEEPSUCK_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.dag/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Dag block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from dag sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.dag/dashboard-plugins/.',
]
