...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or deepsuck -c.",
    ...
    "Ctrl+Z suspends Deepsuck to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "deepsuck -c resumes your most recent CLI session. deepsuck -c \"project name\" resumes by title.",
    "deepsuck -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "deepsuck -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "deepsuck chat -t web,terminal enables only specific toolsets for a focused session.",
    "deepsuck chat -s github-pr-workflow preloads a skill at launch.",
    "deepsuck chat -q \"query\" runs a single non-interactive query and exits.",
    "deepsuck chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "deepsuck chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "deepsuck --yolo bypasses all dangerous command approval prompts for the entire session.",
    "deepsuck chat --source telegram tags the session for filtering in deepsuck sessions list.",
    "deepsuck -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "deepsuck doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "deepsuck dump outputs a compact setup summary — great for bug reports.",
    "deepsuck config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "deepsuck config edit opens config.yaml in your default editor.",
    "deepsuck config check scans for missing or stale configuration options.",
    "deepsuck sessions browse opens an interactive session picker with search.",
    "deepsuck sessions stats shows session counts by platform and database size.",
    "deepsuck sessions prune --older-than 30 cleans up old sessions.",
    "deepsuck skills search react --source skills-sh searches the skills.sh public directory.",
    "deepsuck skills check scans installed hub skills for upstream updates.",
    "deepsuck skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "deepsuck skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "deepsuck mcp add github --command npx adds MCP servers from the command line.",
    "deepsuck mcp serve runs Deepsuck itself as an MCP server for other agents.",
    "deepsuck auth add lets you add multiple API keys for credential pool rotation.",
    "deepsuck completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "deepsuck logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "deepsuck backup creates a zip backup of your entire Deepsuck home directory.",
    "deepsuck profile create coder creates an isolated profile that becomes its own command.",
    "deepsuck profile create work --clone copies your current config and keys to a new profile.",
    "deepsuck update syncs new bundled skills to ALL profiles automatically.",
    "deepsuck gateway install sets up Deepsuck as a system service (systemd/launchd).",
    "deepsuck memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "deepsuck webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: deepsuck tools disables unused tools, deepsuck skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "deepsuck models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as deepsuck -w).",
    ...
    "DEEPSUCK_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Deepsuck tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'deepsuck profile create coder' creates the 'coder' command.",
    "deepsuck profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Deepsuck prints a resume command with session ID and stats.",
    "deepsuck sessions export backup.jsonl exports all sessions for backup or analysis.",
    "deepsuck -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "deepsuck skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .deepsuck/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: deepsuck cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Deepsuck runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "deepsuck gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Deepsuck via a local macOS server.",
    ...
    "Context files (.deepsuck.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Deepsuck loads project context from .deepsuck.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "deepsuck mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "deepsuck mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "deepsuck mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — deepsuck tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.deepsuck/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "deepsuck chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "deepsuck plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "deepsuck acp runs Deepsuck as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "DEEPSUCK_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "deepsuck auth add nous or deepsuck auth add openai-codex sets up OAuth-based providers.",
    ...
    "deepsuck status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.deepsuck/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Deepsuck your own.",
    ...
    "Before a gateway session reset, Deepsuck auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.deepsuck/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Deepsuck state via DEEPSUCK_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "DEEPSUCK_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in DEEPSUCK_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.deepsuck/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'deepsuck profile create ls' would be rejected.",
    "deepsuck profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "deepsuck claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "DEEPSUCK_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via deepsuck tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.deepsuck/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.deepsuck/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Deepsuck auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Deepsuck config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Deepsuck is working.',
    ...
    '/reload reloads ~/.deepsuck/.env into the running session — pick up new API keys without restarting.',
    ...
    "DEEPSUCK_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.deepsuck/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.deepsuck/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'deepsuck curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "deepsuck curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'deepsuck curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'deepsuck auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'DEEPSUCK_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "DEEPSUCK_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'deepsuck dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'deepsuck dashboard embeds the full Deepsuck TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.deepsuck/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.deepsuck/dashboard-plugins/ — no npm build required.',
    ...
    'DEEPSUCK_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'DEEPSUCK_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'DEEPSUCK_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'DEEPSUCK_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.deepsuck/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.deepsuck/skills/ so drop-in skills appear without restarting the session.',
    ...
    'deepsuck -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'deepsuck chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'deepsuck chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'deepsuck chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "deepsuck chat --source tool tags programmatic chats so they don't clutter deepsuck sessions list.",
    'deepsuck dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'deepsuck sessions rename <ID> "new title" renames any past session; deepsuck sessions delete <ID> removes one.',
    'deepsuck import restores a session export or profile archive produced by sessions export or profile export.',
    'deepsuck fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'deepsuck pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'deepsuck setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'deepsuck status --deep runs the full health sweep across every component; plain deepsuck status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'DEEPSUCK_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'DEEPSUCK_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.deepsuck/plugins/ — trust-gated by design.',
    "DEEPSUCK_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'DEEPSUCK_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'DEEPSUCK_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'DEEPSUCK_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'DEEPSUCK_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'DEEPSUCK_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'DEEPSUCK_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'DEEPSUCK_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'DEEPSUCK_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'DEEPSUCK_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'DEEPSUCK_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Deepsuck block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from deepsuck sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.deepsuck/dashboard-plugins/.',
]
