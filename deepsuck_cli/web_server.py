...
WEB_DIST = Path(os.environ["DEEPSUCK_WEB_DIST"]) if "DEEPSUCK_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
async def _lifespan(app: "FastAPI"):
    ...
    if os.getenv("DEEPSUCK_DESKTOP") == "1":
...
# DEEPSUCK_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
_MANAGED_FILES_ROOT_ENV = "DEEPSUCK_DASHBOARD_FILES_ROOT"
...
def _spawn_deepsuck_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "DEEPSUCK_NONINTERACTIVE": "1"},
    }
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _DEEPSUCK_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _DEEPSUCK_OAUTH_FILE.with_name(
        f"{_DEEPSUCK_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _DEEPSUCK_OAUTH_FILE)
        try:
            _DEEPSUCK_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("DEEPSUCK_PORTAL_BASE_URL")
...
def _resolve_chat_argv(
    ...
    ``DEEPSUCK_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``DEEPSUCK_TUI_SIDECAR_URL`` so
    ...
    env.setdefault("DEEPSUCK_TUI_DISABLE_MOUSE", "1")
    ...
    if resume:
        ...
        env["DEEPSUCK_TUI_RESUME"] = resume

    if sidecar_url:
        env["DEEPSUCK_TUI_SIDECAR_URL"] = sidecar_url
...
# DEEPSUCK_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__DEEPSUCK_AUTH_REQUIRED__`` flag lets the SPA pick the right
...
def start_server(
    ...
    # DEEPSUCK_DASHBOARD_READY, open the browser, *then* serve.
    #
