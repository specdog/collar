...
(``DEEPSUCK_SESSION_PLATFORM``, ``DEEPSUCK_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["DEEPSUCK_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("DEEPSUCK_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("DEEPSUCK_SESSION_PLATFORM", "")

    # after
    from gateway.session_context import get_session_env
    platform = get_session_env("DEEPSUCK_SESSION_PLATFORM", "")
"""
...
_SESSION_PLATFORM: ContextVar = ContextVar("DEEPSUCK_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("DEEPSUCK_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("DEEPSUCK_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("DEEPSUCK_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("DEEPSUCK_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("DEEPSUCK_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("DEEPSUCK_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("DEEPSUCK_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("DEEPSUCK_SESSION_MESSAGE_ID", default=_UNSET)
...
_VAR_MAP = {
    "DEEPSUCK_SESSION_PLATFORM": _SESSION_PLATFORM,
    "DEEPSUCK_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "DEEPSUCK_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "DEEPSUCK_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "DEEPSUCK_SESSION_USER_ID": _SESSION_USER_ID,
    "DEEPSUCK_SESSION_USER_NAME": _SESSION_USER_NAME,
    "DEEPSUCK_SESSION_KEY": _SESSION_KEY,
    "DEEPSUCK_SESSION_ID": _SESSION_ID,
    "DEEPSUCK_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``DEEPSUCK_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("DEEPSUCK_SESSION_ID")`` with an ``os.environ`` fallback,
    so both storage paths must move together when the active session changes.
    """
    import os

    os.environ["DEEPSUCK_SESSION_ID"] = session_id
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``DEEPSUCK_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("DEEPSUCK_SESSION_*", default)``.
