"""
Dag MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Dag-specific extra)

Usage:
    dag mcp serve
    dag mcp serve --verbose
...
logger = logging.getLogger("dag.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DAG_HOME."""
    try:
        from dag_constants import get_dag_home
        return get_dag_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DAG_HOME", Path.home() / ".dag")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from dag_constants import get_dag_home
        directory_file = get_dag_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DAG_HOME", Path.home() / ".dag")
...
class EventBridge:
    ...
    This is the Dag equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from dag_constants import get_dag_home
            db_file = get_dag_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DAG_HOME", Path.home() / ".dag")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Dag MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "dag",
        instructions=(
            "Dag Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Dag MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
