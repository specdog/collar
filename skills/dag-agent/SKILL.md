---
name: dag-agent
description: "Configure, extend, or contribute to Collar Agent."
version: 2.1.0
author: Collar Agent + Teknium
license: MIT
...
Collar Agent is an open-source AI agent framework by Nous Research that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Collar works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.

What makes Collar different:

- **Self-improving through skills** — Collar learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
- **Persistent memory across sessions** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
- **Multi-platform gateway** — the same agent runs on Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Email, and 10+ other platforms with full tool access, not just chat.
- **Provider-agnostic** — swap models and providers mid-workflow without changing anything else. Credential pools rotate across multiple API keys automatically.
- **Profiles** — run multiple independent Collar instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Collar for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.

**This skill helps you work with Collar Agent effectively** — setting it up, configuring features, spawning additional agent instances, troubleshooting issues, finding the right commands and settings, and understanding how the system works when you need to extend or contribute to it.
...
```
dag mcp serve            Run Collar as an MCP server
...
dag uninstall            Uninstall Collar
```
...
/snapshot [sub]      Create or restore state snapshots of Collar config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Collar works on across turns until achieved
...
/busy [sub]          Control what Enter does while Collar is working (CLI)
...
/update              Update Collar to latest (gateway)
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_DAG_CORE_TOOLS` is the default bundle most platforms inherit from.
...
Common "why is Collar doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `dag` invocation) because they're read once at startup.
...
By default (`approvals.mode: manual`), Collar prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
## Spawning Additional Collar Instances

Run additional Collar processes as fully independent subprocesses — separate sessions, tools, and environments.
...
Collar uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
Collar runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
Collar's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Collar-installed venv at
...
every Collar tool and most Windows APIs. Prefer forward slashes in code
...
├── cli.py                # Interactive CLI (CollarCLI)
...
**2. Add to `toolsets.py`** → `_DAG_CORE_TOOLS` list.
