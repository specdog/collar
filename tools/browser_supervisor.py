...
One ``CDPSupervisor`` runs per Dag ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "dag-dialog-bridge.invalid"
...
(() => {
  if (window.__dagDialogBridgeInstalled) return;
  window.__dagDialogBridgeInstalled = true;
  const ENDPOINT = "http://dag-dialog-bridge.invalid/";
