...
One ``CDPSupervisor`` runs per Deepsuck ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "deepsuck-dialog-bridge.invalid"
...
(() => {
  if (window.__deepsuckDialogBridgeInstalled) return;
  window.__deepsuckDialogBridgeInstalled = true;
  const ENDPOINT = "http://deepsuck-dialog-bridge.invalid/";
