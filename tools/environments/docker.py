...
from tools.environments.local import _DEEPSUCK_PROVIDER_ENV_BLOCKLIST
...
def _load_deepsuck_env_vars() -> dict[str, str]:
    """Load ~/.deepsuck/.env values without failing Docker command execution."""
    try:
        from deepsuck_cli.config import load_env
...
def _get_active_profile_name() -> str:
    """Return the active Deepsuck profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale deepsuck-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=deepsuck-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Deepsuck process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=deepsuck-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a deepsuck process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Deepsuck
    process leaves its container behind permanently because there's no
    subsequent Deepsuck process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=deepsuck-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=deepsuck-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``DEEPSUCK_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("DEEPSUCK_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using DEEPSUCK_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'deepsuck'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``deepsuck-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.deepsuck/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. deepsuck-agent:latest) already use /init as PID 1
        ...
        container_name = f"deepsuck-{uuid.uuid4().hex[:8]}"
        # Labels make deepsuck-created containers identifiable to:
        #   * the orphan reaper (`deepsuck-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`deepsuck-task-id`, `deepsuck-profile`)
        #   * operators running `docker ps --filter label=deepsuck-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Deepsuck profile is captured at
        ...
        label_args = [
            "--label", "deepsuck-agent=1",
            "--label", f"deepsuck-task-id={task_label}",
            "--label", f"deepsuck-profile={profile_name}",
        ]
        ...
        self._labels = {
            "deepsuck-agent": "1",
            "deepsuck-task-id": task_label,
            "deepsuck-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Deepsuck process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Deepsuck secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _DEEPSUCK_PROVIDER_ENV_BLOCKLIST)
        deepsuck_env = _load_deepsuck_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = deepsuck_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("deepsuck-task-id", "")
        profile_label = self._labels.get("deepsuck-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"deepsuck-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``deepsuck-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=deepsuck-agent=1",
                    "--filter", f"label=deepsuck-task-id={task_label}",
                    "--filter", f"label=deepsuck-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Deepsuck process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Deepsuck exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Deepsuck process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Deepsuck startup. That covers the
        ...
        # outlives Deepsuck processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"deepsuck-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``deepsuck /quit``
