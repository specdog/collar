...
def _load_dag_env_vars() -> dict[str, str]:
    """Load ~/.dag/.env values without failing Docker command execution."""
    try:
        from dag_cli.config import load_env
...
def _get_active_profile_name() -> str:
    """Return the active Dag profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale dag-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=dag-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Dag process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=dag-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a dag process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Dag
    process leaves its container behind permanently because there's no
    subsequent Dag process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=dag-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=dag-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'dag'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``dag-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.dag/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. dag-agent:latest) already use /init as PID 1
        ...
        container_name = f"dag-{uuid.uuid4().hex[:8]}"
        # Labels make dag-created containers identifiable to:
        #   * the orphan reaper (`dag-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`dag-task-id`, `dag-profile`)
        #   * operators running `docker ps --filter label=dag-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Dag profile is captured at
        ...
        label_args = [
            "--label", "dag-agent=1",
            "--label", f"dag-task-id={task_label}",
            "--label", f"dag-profile={profile_name}",
        ]
        ...
        self._labels = {
            "dag-agent": "1",
            "dag-task-id": task_label,
            "dag-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Dag process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Dag secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _DEEPSUCK_PROVIDER_ENV_BLOCKLIST)
        dag_env = _load_dag_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = dag_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("dag-task-id", "")
        profile_label = self._labels.get("dag-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"dag-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``dag-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=dag-agent=1",
                    "--filter", f"label=dag-task-id={task_label}",
                    "--filter", f"label=dag-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Dag process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Dag exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Dag process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Dag startup. That covers the
        ...
        # outlives Dag processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"dag-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``dag /quit``
