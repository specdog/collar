...
running under the dispatcher (env var ``DEEPSUCK_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``deepsuck chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``deepsuck kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``deepsuck kanban complete …``
   inside the container, where ``deepsuck`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.deepsuck/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``deepsuck kanban …``), the dashboard
(``deepsuck dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``DEEPSUCK_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``deepsuck chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("DEEPSUCK_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("DEEPSUCK_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("DEEPSUCK_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``DEEPSUCK_KANBAN_TASK`` set
    ...
    ``DEEPSUCK_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``DEEPSUCK_KANBAN_DB`` → ``DEEPSUCK_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Deepsuck.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``DEEPSUCK_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``DEEPSUCK_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``DEEPSUCK_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``DEEPSUCK_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("DEEPSUCK_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("DEEPSUCK_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("DEEPSUCK_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set DEEPSUCK_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # DEEPSUCK_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``deepsuck-system`` and poison
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # ACP (which sets DEEPSUCK_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("DEEPSUCK_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (DEEPSUCK_KANBAN_TASK set)
    ...
    # (kanban toolset, no DEEPSUCK_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("DEEPSUCK_KANBAN_TASK")
                if _self_tid:
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to DEEPSUCK_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: DEEPSUCK_KANBAN_DB env → "
    "DEEPSUCK_KANBAN_BOARD env → the 'current' symlink under the kanban "
