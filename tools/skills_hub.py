#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Deepsuck Skills Hub.
...
Used by deepsuck_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            deepsuck_meta = metadata.get("deepsuck", {})
            if isinstance(deepsuck_meta, dict):
                tags = deepsuck_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            deepsuck_meta = metadata.get("deepsuck", {})
            if isinstance(deepsuck_meta, dict):
                raw_tags = deepsuck_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.deepsuck/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                deepsuck_meta = meta_block.get("deepsuck", {})
                if isinstance(deepsuck_meta, dict):
                    tags = deepsuck_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Deepsuck centralized index source
# ---------------------------------------------------------------------------

DEEPSUCK_INDEX_URL = "https://deepsuck-agent.nousresearch.com/docs/api/skills-index.json"
DEEPSUCK_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "deepsuck-index.json"
DEEPSUCK_INDEX_TTL = 6 * 3600  # 6 hours


def _load_deepsuck_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Deepsuck index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Deepsuck index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class DeepsuckIndexSource(SkillSource):
    """Skill source backed by the centralized Deepsuck Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_deepsuck_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "deepsuck-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "deepsuck-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        DeepsuckIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "deepsuck-index"
