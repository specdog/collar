#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the DAG Skills Hub.
...
Used by dag_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.dag/skills/ during setup.  They are discoverable via the Skills Hub
...
def _load_dag_index() -> Optional[dict]:
    ...
    try:
        ...
        logger.debug("DAG index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class DagIndexSource(SkillSource):
    """Skill source backed by the centralized DAG Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "dag-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "dag-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        DagIndexSource(auth=auth), # Centralized index (search + resolved install paths)
